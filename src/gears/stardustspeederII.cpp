...
void Player_SSIIRingBoost(Player *player) {
	if (player->input->toggleFaceButtons.hasAny(XButton) &&
	   ...
	   player->unkB90 <= 0
		...
		if(player->characterArchetype == BoostArchetype) { player->gearStats[player->level].boostSpeed = pSpeed(320) + BoostArchetypeBoostSpeeds[player->level]; }
		player->stardustspeederII_1frameboost = TRUE;
	} else if((player->movementFlags & boosting) == 0u) {
		player->gearStats[player->level].boostSpeed = pSpeed(245);
		if(player->characterArchetype == BoostArchetype) { player->gearStats[player->level].boostSpeed = pSpeed(245) + BoostArchetypeBoostSpeeds[player->level]; }
	}
}