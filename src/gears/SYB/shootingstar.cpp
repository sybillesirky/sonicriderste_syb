...
void Player_ShootingStar(Player *player) {
	...
	if (player->SYBArSShSCounter > 20) {
		player->SYBArSShSCounter = 20;
	}
	...
	if (player->state == FrontflipRamp || player->state == BackflipRamp || player->state == ManualRamp || player->state == HalfPipeTrick) {
		player->SYBBeenTricking = true; // This makes sure trick landing behaviours only fire once.
	}
	...
	if (player->previousState == FrontflipRamp || player->previousState == BackflipRamp || player->previousState == ManualRamp || player->previousState == HalfPipeTrick) {
		if (player->state == Cruise || player->state == Fly || player->state == RailGrind) {
			player->SYBArSShSCounter += player->trickCount;

			// If trick rank is lower than X, induce penalties if level 2 or higher.
			if (player->trickCount < 4 && player->SYBBeenTricking == true) {
				if (player->level > 0) {
		...
		if (player->SYBBeenTricking == true) { // Prevents constant updating.
			if (player->state == Cruise || player->state == Fly || player->state == RailGrind) {
				player->SYBShSRsTLevelTracker = player->level;

				// Update the level and stats now that we have the new amount of tricks.
				if (player->SYBArSShSCounter >= 20) { // Level 3
					player->level = 2;
				} else if (player->SYBArSShSCounter >= 10) { // Level 2
					player->level = 1;
				} else { // Level 1
					player->level = 0;
				}	
		...
		player->SYBBeenTricking = false;
	}

	if (player->state == StartLine) { // Initialising behaviours.
		player->SYBArSShSCounter = 0;
